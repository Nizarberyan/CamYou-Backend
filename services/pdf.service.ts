...
const PDFService = {
  ...
  generateDailyReportPDF: (report: any, res: Response) => {
    const doc = new PDFDocument({ margin: 50 });
    const dateStr = new Date(report.date).toISOString().split("T")[0];
    const filename = `DailyReport-${dateStr}.pdf`;
    ...
    // Header
    doc.fontSize(24).text("Daily Fleet Report", { align: "center" });
    doc.fontSize(12).text(new Date(report.date).toDateString(), { align: "center" });
    doc.moveDown(2);

    // Summary Box
    doc.rect(50, 150, 500, 130).stroke();

    doc.fontSize(16).text("Summary Statistics", 70, 170);
    doc.moveDown();
    ...
    // Stats Grid
    doc.fontSize(12).text(`Total Distance:`, col1X, startY);
    doc.font('Helvetica-Bold').text(`${report.totalMiles.toLocaleString()} km`, col1X + 120, startY);

    doc.font('Helvetica').text(`Total Fuel Used:`, col1X, startY + 25);
    doc.font('Helvetica-Bold').text(`${report.totalFuel.toLocaleString()} L`, col1X + 120, startY + 25);

    doc.font('Helvetica').text(`Completed Trips:`, col2X, startY);
    doc.font('Helvetica-Bold').text(`${report.completedTrips}`, col2X + 120, startY);

    doc.font('Helvetica').text(`Active Trips:`, col2X, startY + 25);
    doc.font('Helvetica-Bold').text(`${report.activeTrips}`, col2X + 120, startY + 25);

    doc.moveDown(5);

    // Additional Info or Breakdown could go here

    // Footer
    const bottom = doc.page.height - 50;
    doc.fontSize(10).font('Helvetica').text("Generated by CamYou Fleet Manager", 50, bottom, { align: "center", width: 500 });
